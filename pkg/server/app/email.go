...
func (a *App) SendVerificationEmail(email, tokenValue string) error {
	...
	if err := a.EmailBackend.Queue("Verify your nad email address", from, []string{email}, mailer.EmailKindText, body); err != nil {
...
func (a *App) SendWelcomeEmail(email string) error {
	...
	if err := a.EmailBackend.Queue("Welcome to nad!", from, []string{email}, mailer.EmailKindText, body); err != nil {
		return errors.Wrapf(err, "queueing email for %s", email)
	}
...
func (a *App) SendPasswordResetAlertEmail(email string) error {
	...
	if err := a.EmailBackend.Queue("nad password changed", from, []string{email}, mailer.EmailKindText, body); err != nil {
		return errors.Wrapf(err, "queueing email for %s", email)
	}
...
func (a *App) SendSubscriptionConfirmationEmail(email string) error {
	...
	if err := a.EmailBackend.Queue("Welcome to nad Pro", from, []string{email}, mailer.EmailKindText, body); err != nil {
		return errors.Wrapf(err, "queueing email for %s", email)
	}
