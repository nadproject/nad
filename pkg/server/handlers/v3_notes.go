...
func (c *Context) UpdateNote(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	...
	if err := c.App.DB.Where("uuid = ? AND user_id = ?", noteUUID, user.ID).First(&note).Error; err != nil {
	...
	note, err = c.App.UpdateNote(tx, user, note, &app.UpdateNoteParams{
...
func (c *Context) DeleteNote(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	noteUUID := vars["noteUUID"]
	...
	var note database.Note
	if err := c.App.DB.Where("uuid = ? AND user_id = ?", noteUUID, user.ID).Preload("Book").First(&note).Error; err != nil {
		HandleError(w, "finding note", err, http.StatusInternalServerError)
		return
	}
	...
	n, err := c.App.DeleteNote(tx, user, note)
	if err != nil {
		tx.Rollback()
...
func (c *Context) CreateNote(w http.ResponseWriter, r *http.Request) {
	user, ok := r.Context().Value(helpers.KeyUser).(database.User)
	if !ok {
		HandleError(w, "No authenticated user found", nil, http.StatusInternalServerError)
		return
	...
	var book database.Book
	if err := c.App.DB.Where("uuid = ? AND user_id = ?", params.BookUUID, user.ID).First(&book).Error; err != nil {
		HandleError(w, "finding book", err, http.StatusInternalServerError)
		return
	}

	note, err := c.App.CreateNote(user, params.BookUUID, params.Content, params.AddedOn, params.EditedOn, false)
	if err != nil {
...
func (c *Context) NotesOptions(w http.ResponseWriter, r *http.Request) {
