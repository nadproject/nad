...
func (c *Context) getMe(w http.ResponseWriter, r *http.Request) {
	...
	tx := c.App.DB.Begin()
	if err := c.App.TouchLastLoginAt(user, tx); err != nil {
		tx.Rollback()
...
func (c *Context) createResetToken(w http.ResponseWriter, r *http.Request) {
	...
	resetToken, err := token.Create(c.App.DB, user.ID, database.TokenTypeResetPassword)
	if err != nil {
		HandleError(w, errors.Wrap(err, "generating token").Error(), nil, http.StatusInternalServerError)
		return
	}

	if err := c.App.SendPasswordResetEmail(user.Email, resetToken.Value); err != nil {
...
func (c *Context) resetPassword(w http.ResponseWriter, r *http.Request) {
	...
	conn := c.App.DB.Where("value = ? AND type =? AND used_at IS NULL", params.Token, database.TokenTypeResetPassword).First(&token)
	if conn.RecordNotFound() {
	...
	var user database.User
	if err := c.App.DB.Where("user_id = ?", token.UserID).First(&user).Error; err != nil {
		tx.Rollback()
	...
	if err := c.App.SendPasswordResetAlertEmail(user.Email); err != nil {
