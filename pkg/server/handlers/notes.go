...
func (c *Context) getNote(w http.ResponseWriter, r *http.Request) {
	...
	note, ok, err := operations.GetNote(c.App.DB, noteUUID, user)
	if !ok {
		RespondNotFound(w)
		return
	}
...
func (c *Context) getNotes(w http.ResponseWriter, r *http.Request) {
	...
	respondGetNotes(c.App.DB, user.ID, query, w)
}
...
func (c *Context) legacyGetNotes(w http.ResponseWriter, r *http.Request) {
	user, ok := r.Context().Value(helpers.KeyUser).(database.User)
	if !ok {
		HandleError(w, "No authenticated user found", nil, http.StatusInternalServerError)
		return
	...
	if err := c.App.DB.Where("user_id = ? AND encrypted = true", user.ID).Find(&notes).Error; err != nil {
		HandleError(w, "finding notes", err, http.StatusInternalServerError)
		return
	}
