...
func (c *Context) CreateBook(w http.ResponseWriter, r *http.Request) {
	...
	var bookCount int
	err = c.App.DB.Model(database.Book{}).
	...
	book, err := c.App.CreateBook(user, params.Name)
	if err != nil {
...
func (c *Context) BooksOptions(w http.ResponseWriter, r *http.Request) {
...
func (c *Context) GetBooks(w http.ResponseWriter, r *http.Request) {
	user, ok := r.Context().Value(helpers.KeyUser).(database.User)
	if !ok {
		return
	}
	...
	respondWithBooks(c.App.DB, user.ID, query, w)
}

// GetBook returns a book for the user
func (c *Context) GetBook(w http.ResponseWriter, r *http.Request) {
	user, ok := r.Context().Value(helpers.KeyUser).(database.User)
	if !ok {
		return
	}
	...
	conn := c.App.DB.Where("uuid = ? AND user_id = ?", bookUUID, user.ID).First(&book)
...
func (c *Context) UpdateBook(w http.ResponseWriter, r *http.Request) {
	user, ok := r.Context().Value(helpers.KeyUser).(database.User)
	if !ok {
		return
	}
	...
	book, err = c.App.UpdateBook(tx, user, book, params.Name)
	if err != nil {
		tx.Rollback()
...
func (c *Context) DeleteBook(w http.ResponseWriter, r *http.Request) {
	user, ok := r.Context().Value(helpers.KeyUser).(database.User)
	if !ok {
		return
	}
	...
	for _, note := range notes {
		if _, err := c.App.DeleteNote(tx, user, note); err != nil {
			HandleError(w, "deleting a note", err, http.StatusInternalServerError)
			return
		}
	}
	b, err := c.App.DeleteBook(tx, user, book)
	if err != nil {
